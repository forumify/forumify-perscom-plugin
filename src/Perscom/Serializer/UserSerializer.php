...
class UserSerializer implements DenormalizerInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): PerscomUser
    {
        ...
        if (!empty($data['position_id'])) {
            $user->setPosition($context['positions'][$data['position_id']] ?? null);
        }

        if (!empty($data['rank_id'])) {
            $user->setRank($context['ranks'][$data['rank_id']] ?? null);
        }

        if (!empty($data['specialty_id'])) {
            $user->setSpecialty($context['specialties'][$data['specialty_id']] ?? null);
        }

        if (!empty($data['status_id'])) {
            $user->setStatus($context['statuses'][$data['status_id']] ?? null);
        }

        if (!empty($data['unit_id'])) {
            $user->setUnit($context['units'][$data['unit_id']] ?? null);
        }
